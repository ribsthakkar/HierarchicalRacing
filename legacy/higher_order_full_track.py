...
# while t <= time_horizon+time_delta/2:
#     if t > time_horizon-time_delta/2: t = time_horizon
#     con = lambda c: find_pos_index(init_position_index, trajectory(c[:len(c_x)], t), trajectory(c[len(c_x):len(c_x)+len(c_y)], t)) \
#                     - find_pos_index(init_position_index, trajectory(c[:len(c_x)], t-time_delta), trajectory(c[len(c_x):len(c_x)+len(c_y)], t-time_delta))
#     nlc = NonlinearConstraint(con, 0, np.inf)
#     constraints.append(nlc)
#     t += time_delta
