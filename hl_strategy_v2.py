...
class TrackDef:
    ...
    def generate_guards_and_updates(self, position, line, velocity, tp_update_string, car_def, max_time, mdp_tire_age_limit, lap_change):
            ...
            for ta in range(0, MAX_TIRE_AGE, tire_wear_step):
                for init_v in range(1, car_def.max_v+1, velocity_step):
                    ...
                    for target_v in feasible_v:
                        est_dt = int(math.ceil(calc_min_time(avg_init_v, target_v, dist, car_def.max_braking, car_def.max_acceleration, car_def.max_v)))
                        ...
                        changes = f"{tp_update_string} & (velocity'={target_v}) & (track_line'={line}) & (tire_age'=tire_age+{est_dta}) & (t'=t+{est_dt}) {lap_change_update}"
                        updates.append((prob, changes))
                    guard = f"track_line={tl} & {ta}<=tire_age & tire_age<{ta + tire_wear_step} & {init_v}<=velocity & velocity<{init_v + velocity_step} & t<={max_time - max_dt} & tire_age<={mdp_tire_age_limit - max_dta}"
                    if len(updates):
...
def generate_racecar_module(output_file, max_time, laps, track_definition, car_definition):
    with open(output_file, "w+") as output:
        ...
        _write_with_newline_and_sc(f'track_line : [0..{max(1, tls-1)}] init {init_line}', output)
        ...
        for track_pos in range(0, tps):
            ...
            for line in range(0, tls):
                for velocity in range(1, max_v+1, car_def.velocity_step):
                    action = f"[step{track_pos}_{line}_{velocity}_{velocity+car_def.velocity_step}]"
                    for ta_v_guard, updates in track_definition.generate_guards_and_updates(track_pos, line, velocity, tp_update_string, car_definition, max_time, MAX_TIRE_AGE+100, lap_change):
...
if __name__ == "__main__":
    ...
    car_def = CarDef(max_velocity=10, velocity_step=2, min_gs=0.3*9.8, max_gs=1*9.8, max_braking=5, max_acceleration=5,
                     tire_wear_factor=1,
                     init_time=0, init_tire=0, init_line=0, init_velocity=3, init_position=0)
    print(datetime.now())
    print("Generating Prism Program...")
    generate_racecar_module('result.txt', max_time=250, laps=3, track_definition=track_def, car_definition=car_def)
