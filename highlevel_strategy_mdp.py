...
class TrackComponent:
    ...
    def tire_wear(self, velocity, line):
        raise NotImplementedError("Must be implemented by child classes")
            ...
            for ta in range(0, MAX_TIRE_AGE, tire_wear_step):
                for v in range(0, car_def.max_v, velocity_step):
                    ...
                    for fv in feasible_v:
                        ...
                        est_dta = entry.tire_wear(fv, tl)
                        prob = f'1/{len(feasible_v)}'
                        changes = f"{tp_update_string} & (velocity'={fv}) & (track_line'={line}) & (tire_age'=tire_age+{est_dta}) & (t'=t+{est_dt}) & (dt'={est_dt}) {lap_change_update}"
                ...
                if acc <= 0:
                    ...
                    if len(updates):
                        output.append((guard, updates))
            updates = []
            guard = f"tire_age >={MAX_TIRE_AGE}"
            est_dt = int(straight.length / 1)
            prob = '1'
            changes = f"{tp_update_string} & (velocity'={1}) & (track_line'={line}) & (tire_age'=tire_age) & (t'=t+{est_dt}) & (dt'={est_dt}) {lap_change_update}"
            updates.append((prob, changes))
            output.append((guard, updates))
        return output
...
def generate_racecar_module(output_file, max_time, laps, track_definition, car_definition):
    with open(output_file, "w+") as output:
        ...
        _write_with_newline_and_sc(f"lap<{laps}: dt", output)
...
if __name__ == "__main__":
    ...
    generate_racecar_module('result.txt', max_time=100, laps=3, track_definition=track_def, car_definition=car_def)
