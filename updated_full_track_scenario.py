...
def calc_max_point_horizon(currx, curry, currdx, currdy, currd2x, currd2y, ipx):
    maxx = currdx + 0.5*currd2x
    maxy = currdy + 0.5*currd2y
    max_dist = math.sqrt(maxx**2 + maxy**2)
...
init_d2x_dt = -2
init_d2y_dt = -10
init_position_index = 2
init_x = main_track_x[init_position_index]
init_y = main_track_y[init_position_index]
min_point_horizon = 1 #max(int(math.sqrt(init_dx_dt**2 + init_dy_dt**2)), 1)
max_point_horizon = calc_max_point_horizon(init_x, init_y, init_dx_dt, init_dy_dt, init_d2x_dt, init_d2y_dt, init_position_index)
...
c_x = [init_x, (init_x+init_dx_dt/5), init_d2x_dt/20 + 2 * ((init_x+init_dx_dt/5)) - init_x, init_x, init_x, main_track_x[init_position_index+min_point_horizon]]
c_y = [init_y, (init_y+init_dy_dt/5), init_d2y_dt/20 + 2 * ((init_y+init_dy_dt/5)) - init_y, init_y, init_y, main_track_y[init_position_index+min_point_horizon]]
c = c_x + c_y
...
def arc_length(cx, cy, delta=0.05):
    ...
    d = 0.0
    while tau1 < 1:
        update = dist(trajectory_x(cx, tau2), trajectory_y(cy, tau2), trajectory_x(cx, tau1), trajectory_y(cy, tau1))
        d += update
...
def opt(c):
    ...
    while t <= 1:
        ...
        # if ((x_p * x_p + y_p * y_p) > 0):
        #     proj = (x_p * x_pp + y_p * y_pp) / (x_p * x_p + y_p * y_p)
        # else:
        #     proj = 0
        # if proj <= 0:
        #     total -= 1000* (proj * math.sqrt(x_p ** 2 + y_p ** 2) + 7 * gravitational_acceleration)
        # else:
        #     total -= 1000* (-0.1803 * math.sqrt(x_p ** 2 + y_p ** 2) + 21.008 - proj * math.sqrt(x_p ** 2 + y_p ** 2))
        total += 100*distance_to_center(trajectory_x(c[:len(c_x)], t), trajectory_y(c[len(c_x):len(c_x)+len(c_y)], t))
        t += time_delta
    ...
    total += 10*abs(c[len(c_x)-1] - main_track_x[(int(c[-1]) + init_position_index)%len(main_track_x)])
    total += 10*abs(c[len(c_x) + len(c_y)-1] - main_track_y[(int(c[-1]) + init_position_index)%len(main_track_y)])
    return total - (c[-1]-init_position_index) -math.sqrt(speed_x(c[:len(c_x)], 1)**2 + speed_y(c[len(c_x):len(c_x)+len(c_y)], 1)**2)
...
while t <= 1:
    ...
    nlc = NonlinearConstraint(con, 0, 15)
    constraints.append(nlc)
...
#         y_p = speed_y(c[len(c_x):len(c_x) + len(c_y)], t)
#         y_pp = acceleration_y(c[len(c_x):len(c_x) + len(c_y)], t)
#         if ((x_p * x_pp + y_p * y_pp) < 0):
#             return 7*gravitational_acceleration - math.sqrt(x_pp ** 2 + y_pp ** 2)
#         else:
#             return -0.1803 * math.sqrt(x_p ** 2 + y_p ** 2) + 21.008 - math.sqrt(x_pp ** 2 + y_pp ** 2)
#     nlc = NonlinearConstraint(con, 0, np.inf)
#     constraints.append(nlc)
#     # print(t, con(initial))
#     t += time_delta
#
# con = lambda c:  arc_length(c[:len(c_x)], c[len(c_x):len(c_x)+len(c_y)])
# nlc = NonlinearConstraint(con, 100, np.inf)
# constraints.append(nlc)
...
print(min_point_horizon, max_point_horizon)
bounds = Bounds([init_x, (init_x+init_dx_dt/5), init_d2x_dt/20 + 2 * ((init_x+init_dx_dt/5)) - init_x, 0, 0, 0,
                 init_y, (init_y+init_dy_dt/5), init_d2y_dt/20 + 2 * ((init_y+init_dy_dt/5)) - init_y, 0, 0, 0, min_point_horizon],

                [init_x, (init_x+init_dx_dt/5), init_d2x_dt/20 + 2 * ((init_x+init_dx_dt/5)) - init_x, 800, 800, 800,
                 init_y, (init_y+init_dy_dt/5), init_d2y_dt/20 + 2 * ((init_y+init_dy_dt/5)) - init_y, 600, 600, 600, max_point_horizon])

result = minimize(opt, initial, bounds=bounds, constraints=constraints, options={'disp': True, 'maxiter':250})
...
print("final speed x", speed_x(output[:len(c_x)], 1 ), "final speed y", speed_y(output[len(c_x):len(c_x)+len(c_y)], 1))
print("final acc x", acceleration_x(output[:len(c_x)], 1 ), "final acc y", acceleration_y(output[len(c_x):len(c_x)+len(c_y)], 1))
...
#     # nlc = NonlinearConstraint(con, 0, (track_width/2)-car_width)
#     # constraints.append(nlc)
#     print("t=",t, "closest_distance_to_center_line=", con(output), "x=",trajectory_x(output[:len(c_x)], t),"y=", trajectory_y(output[len(c_x):len(c_x)+len(c_y)], t))
#     t += time_delta
...
while t <= 1.05:
    ...
    y_p = speed_y(c[len(c_x):len(c_x) + len(c_y)], t)
    y_pp = acceleration_y(c[len(c_x):len(c_x) + len(c_y)], t)
    speed = math.sqrt(x_p**2 + y_p**2)
    acc = math.sqrt(x_pp**2 + y_pp**2)
    # print("time=", t, "speed=", speed, "sp_x=", speed_x(output[:len(c_x)], t), "sp_y=", speed_y(output[len(c_x):len(c_x)+len(c_y)], t))
    if ((x_p * x_pp + y_p * y_pp) < 0):
        print("time=", t, "acc_bound=", 7*gravitational_acceleration, "acc=", acc ,"constraint3_val=", con3(output), "acc_x=", acceleration_x(output[:len(c_x)], t), "acc_y=", acceleration_y(output[len(c_x):len(c_x)+len(c_y)], t))
    else:
        print("time=", t, "acc_bound=", -0.1803 * speed + 21.008, "acc=", acc ,"constraint3_val=", con3(output), "acc_x=", acceleration_x(output[:len(c_x)], t), "acc_y=", acceleration_y(output[len(c_x):len(c_x)+len(c_y)], t))
...
#     print("time=", t, "lateral_acceleration=", con(output), "acc_x=", acceleration_x(output[:len(c_x)], t), "acc_y=", acceleration_y(output[len(c_x):len(c_x)+len(c_y)], t))
#     t += time_delta
#
#
