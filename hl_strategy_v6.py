...
class TrackStraight(TrackComponent):
    def __init__(self, length, num_lines, component_wear_factor=.0125):
                    ...
                    for action, action_string in action_set.items():
                        ...
                        for v in range(action[0], action[1]):
                            ...
                            for section_idx, section in enumerate(track_definition.landmarks):
                                if action in not_allowed_actions[section_idx]: continue
                                dist = math.sqrt(
...
if __name__ == "__main__":
    ...
    ## SCENARIO 2
    components = [TrackStraight(30, LANES), TrackStraight(30, LANES), TrackCorner(num_lines=LANES, width=WIDTH, inside_turn_radius=25, degrees=180, exit_length=30, left_turn=False),]
    ...
    car_def1 = CarDef(min_velocity=20, max_velocity=70, main_velocity_step=10, init_velocity_step=5, min_gs=2*9.8, max_gs=3*9.8, max_braking=20, max_acceleration=9,
                     tire_wear_factor=.08,
                     init_time=0, init_tire=0, init_line=0, init_velocity=1, init_position=0)
    car_def2 = CarDef(min_velocity=20, max_velocity=65, main_velocity_step=10, init_velocity_step=5, min_gs=2*9.8, max_gs=3.5*9.8, max_braking=20, max_acceleration=12,
                     tire_wear_factor=.1,
