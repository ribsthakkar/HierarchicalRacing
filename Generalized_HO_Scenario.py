...
def opt(c):
    ...
    while t <= time_horizon + time_delta / 2:
        ...
        total -= .25 * sp
        total += 1 * distance_to_center(trajectory(c[:len(c_x)], t),
                                          trajectory(c[len(c_x):len(c_x) + len(c_y)], t))
...
for i in range(1,31):
    print(f"***---ROUND {i}---***")
    print("Initial x speed", speed(initial[:len(c_x)], 0), "Initial y speed", speed(initial[len(c_x):len(c_x) + len(c_y)], 0),
          "Initial x acc", acceleration(initial[:len(c_x)], 0), "Initial y acc", acceleration(initial[len(c_x):len(c_x) + len(c_y)], 0))
    ...
    fin_speed_x = speed(output[:len(c_x)], time_horizon-1)
    fin_speed_y = speed(output[len(c_x):len(c_x)+len(c_y)], time_horizon-1)
    fin_speed = math.sqrt(fin_speed_x**2 + fin_speed_y**2)
    fin_acc_x = acceleration(output[:len(c_x)], time_horizon-1)
    fin_acc_y = acceleration(output[len(c_x):len(c_x)+len(c_y)], time_horizon-1)


    print("c_x", output[:len(c_x)])
    print("c_y", output[len(c_x):len(c_x)+len(c_y)])
    print("N", output[-1])
    print("trajectory distance:", arc_length(output[:len(c_x)], output[len(c_x):len(c_x)+len(c_y)], time_horizon=time_horizon-1))
    print("final speed", fin_speed)
    print("final speed x", fin_speed_x, "final speed y", fin_speed_y)
    print("final acc x", fin_acc_x, "final acc y", fin_acc_y)
    x = trajectory(output[:len(c_x)], time_horizon-1)
    y = trajectory(output[len(c_x):len(c_x) + len(c_y)], time_horizon-1)
    final_pos_idx = find_pos_index(init_position_index, x, y)
    print("t=", time_horizon-1, "closest_distance_to_center_line=", distance_to_center(x, y), "x=", x, "y=", y, "pos_index=", final_pos_idx)

    init_dx_dt = fin_speed_x  # 13.2# 0
    init_dy_dt = fin_speed_y  # -41.0# 0
    init_d2x_dt = fin_acc_x  # 2#-14
    init_d2y_dt = fin_acc_y  # -13# -14
    init_position_index = final_pos_idx  # 41# 1
    init_x = x
    init_y = y
    track_horizon = np.array([*zip(main_track_x, main_track_y)])
    line = geom.LineString(track_horizon)
    ...
    while t <= time_horizon+time_delta/2:
        ...
        total_d_from_center += abs(con(output))
    ...
    while t <= time_horizon+time_delta/2:
        ...
        t += time_delta
    control_points.append(output)
    print("total absolute dist from center", total_d_from_center)
    print("total_constraint3_val", total_c3_val)
...
for cp in control_points:
    t = 0
    while t <= time_horizon-1+time_delta/2:
        if t > time_horizon-1-time_delta/2: t = time_horizon-1
        x = trajectory(cp[:len(c_x)], t)
        y = trajectory(cp[len(c_x):len(c_x) + len(c_y)], t)
        planned_motion_x.append(x)
        planned_motion_y.append(y)
        t += time_delta


plt.plot(main_track_x, main_track_y, '.r-')
plt.plot(planned_motion_x, planned_motion_y, '.b-')
plt.savefig("run_"+str(datetime.datetime.now()) + ".png")
plt.show()
