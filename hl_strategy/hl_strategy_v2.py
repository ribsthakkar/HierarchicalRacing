...
class TrackCorner:
    ...
    def __init__(self, num_lines, width, inside_turn_radius, entry_length, mid_length, exit_length, left_turn=True):
        self.entry = self.Entry(entry_length, inside_turn_radius, width, num_lines, left_turn)
        self.mid = self.Mid(mid_length, inside_turn_radius, width, num_lines, left_turn)
...
class TrackDef:
    ...
    def generate_guards_and_updates(self, idx, position, line, velocity, tp_update_string, car_def, max_time, mdp_tire_age_limit, lap_change):
        tail = self.landmarks[position % self.track_points]
        head = self.landmarks[(position+1) % self.track_points]
        lap_change_update = "" if not lap_change else f"& (lap{idx}'=lap{idx}+1)"
        output = []
        tire_wear_step = 10
            ...
            for ta in range(0, MAX_TIRE_AGE, tire_wear_step):
                for init_v in range(1, car_def.max_v+1, velocity_step):
                    ...
                    for target_v in range(velocity, min(car_def.max_v+1, velocity+velocity_step)):
                        ...
                        feasible_v.append(target_v)
                    max_dt = 0
                    max_dta = 0
                    for target_v in feasible_v:
                        ...
                        changes = f"{tp_update_string} & (velocity{idx}'={target_v}) & (track_line{idx}'={line}) & (tire_age{idx}'=tire_age{idx}+{est_dta}) & (t{idx}'=t{idx}+{est_dt}) {lap_change_update}"
        ...
        prob = '1'
        changes = f"{tp_update_string} & (velocity{idx}'={1}) & (track_line{idx}'={line}) & (tire_age{idx}'=tire_age{idx}) & (t{idx}'=t{idx}+{est_dt}) {lap_change_update}"
...
def generate_racecar_module(output_file, max_time, laps, track_definition, car_definitions):
    with open(output_file, "w+") as output:
        ...
        for idx, car_definition in enumerate(car_definitions):
            ...
            for track_pos in range(0, tps):
                ...
                tp_update_string = f"(track_pos{idx}'=track_pos{idx}+1)" if not lap_change  else f"(track_pos{idx}'=0)"
                for line in range(0, tls):
                    for velocity in range(1, max_v+1, car_definition.velocity_step):
                        ...
                        for ta_v_guard, updates in track_definition.generate_guards_and_updates(idx, track_pos, line, velocity, tp_update_string, car_definition, max_time, MAX_TIRE_AGE+100, lap_change):
                            if len(updates) == 0: continue
                            guard = f"track_pos{idx}={track_pos} & {ta_v_guard} & lap{idx}<{laps}"
                            _write_with_newline_and_sc(f"{action} {guard} -> {'+'.join(map(lambda pair: f'{pair[0]}:{pair[1]}', updates))}", output)

            action = f"[pit_{idx}]"
            guard = f"track_pos{idx}={tps-1} & lap{idx}<{laps} & t{idx} <{max_time-track_def.pit_time}"
            _write_with_newline_and_sc(
                f"{action} {guard} -> 1:(track_pos{idx}'={track_def.pit_exit_p}) & (tire_age{idx}'=0) & (velocity{idx}'={min(track_def.pit_exit_v, max_v)}) & (track_line{idx}'={track_def.pit_exit_l}) & (t{idx}'=t{idx}+{track_def.pit_time}) & (lap{idx}'=lap{idx}+1)", output)
...
if __name__ == "__main__":
    ...
    car_def1 = CarDef(max_velocity=10, velocity_step=2, min_gs=0.3*9.8, max_gs=1*9.8, max_braking=5, max_acceleration=5,
                     tire_wear_factor=1,
                     init_time=0, init_tire=0, init_line=0, init_velocity=3, init_position=0)
    car_def2 = CarDef(max_velocity=10, velocity_step=2, min_gs=0.3*9.8, max_gs=1*9.8, max_braking=5, max_acceleration=5,
                     tire_wear_factor=1,
                     init_time=0, init_tire=0, init_line=1, init_velocity=3, init_position=0)
    print(datetime.now())
    print("Generating Prism Program...")
    generate_racecar_module('result.txt', max_time=250, laps=3, track_definition=track_def, car_definitions=[car_def1, car_def2])
