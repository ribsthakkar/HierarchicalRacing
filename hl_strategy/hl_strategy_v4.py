...
def calc_min_time(u, v, s, a_min, a_max, v_max):
    ...
    if (is_greater_than(max_vel_constraint(result.x), v_max, abs_tol=1e-4) or not math.isclose(distance_constraint(result.x), s, abs_tol=1e-4)
        ...
        # print()
        return None
    # print("u=",u, "v=",v, "s=", s, result.x[3], result.x, distance_constraint(result.x))
    return result.fun
...
class TrackStraight(TrackComponent):
    def __init__(self, length, num_lines):
        super().__init__([length] * num_lines)
...
class TrackCorner:
    class Entry(TrackComponent):
        def __init__(self, lengths, turn_radius, width, num_lines, left_turn=True):
        ...
        def tire_wear(self, velocity, line, tire_wear_factor):
            gs = (velocity**2)/self.tr[line]
            return int((math.ceil(gs) * self.lengths[line]/velocity)/tire_wear_factor)
    ...
    class Mid(TrackComponent):
        ...
        def tire_wear(self, velocity, line, tire_wear_factor):
            gs = (velocity**2)/self.tr[line]
            return math.ceil((math.ceil(gs) * self.lengths[line]/velocity)/tire_wear_factor)
    ...
    def __init__(self, num_lines, width, inside_turn_radius, degrees, exit_length, left_turn=True):
        ...
        self.entry = self.Entry([turn_length/2 for turn_length in turn_lengths], inside_turn_radius, width, num_lines, left_turn)
...
def generate_modules(output_file, total_seconds, laps, track_definition, car_definitions, time_precision, crash_tolerance=1, is_game=False, game_type='smg', allow_worn_progress=True):
    with open(output_file, "w+") as output:
        ...
        for idx, car_definition in enumerate(car_definitions):
            ...
            for i in range(tps):
                active_section_strings[i] = f"section{i}_active_c{idx}"
                _write_with_newline_and_sc(f"formula {active_section_strings[i]} = track_pos{idx}={i} ? 1 : 0", output)
                    ...
                    for action, action_string in action_set.items():
                        ...
                        for v in range(action[0], action[1]):
                            ...
                            for section_idx, section in enumerate(track_definition.landmarks):
                                ...
                                pos_guard_str += f" | track_pos{idx}={section_idx}"
                            updates.append(f"(velocity{idx}'={v})&(track_lane{idx}'={action[2]})&({t_update_str})")
                        prob_str = f"1/{len(updates)}"
        ...
        if game_type == 'smg':
            ...
            for i in range(len(car_definitions)):
                _write_with_newline_and_sc(f"turn{i}: bool init {'true' if i == 0 else 'false'}", output)
            ...
            _write_with_newline_and_sc(f"[] {'&'.join(map(lambda i: f'turn{i}', len(car_definitions)))} & track_pos = {tps-1} -> (track_pos'=0)", output)
            for i in range(len(car_definitions)):
                if len(car_definitions) > 1:
                    ...
                    _write_with_newline_and_sc(
                        f"[] turn{i}=true & track_pos < {tps - 1} -> (track_pos'=track_pos+1) & (turn{i}'=false)",
                        output)
                    _write_with_newline_and_sc(
                        f"[] turn{i}=true & track_pos = {tps - 1} -> (track_pos'=0) & (turn{i}'=false) & (lap'=lap+1",
                        output)
